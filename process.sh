...
if [[ " ${params_array[*]} " != *" merge "* ]]; then
  ...
  do
    ...
    if [[ " ${year} " == *" 00 "* ]]; then
      ...
      ogr2ogr -dialect sqlite -sql "select STATE || COUNTY || CASE WHEN length(TRACT) = 4 THEN TRACT || '00' ELSE TRACT END as GEOID, geometry FROM $RAW_TRACTS_1" $RAW_TRACTS.shp $RAW_TRACTS_1.shp -progress
    elif [[ " ${year} " == *" 10 "* ]]; then
...
if [[ " ${params_array[*]} " != *" filter "* ]]; then
  ...
  do
    ...
    if [[ " ${year} " == *" 00 "* ]] || [[ " ${year} " == *" 10 "* ]] ; then
      ogr2ogr -sql "SELECT DEC_${year}_SF1 as geo_id, DEC_${year}_S_1 as geo_id_two, DEC_${year}_S_2 as geo_label, cast(DEC_${year}_S_3 as float(10,0)) as total_pop FROM $EDIT_ONE" $EDIT_TWO.shp $EDIT_ONE.shp -progress
    else
      ogr2ogr -sql "SELECT ACS_${year}_5YR as geo_id, ACS_${year}_5_1 as geo_id_two, ACS_${year}_5_2 as geo_label, cast(ACS_${year}_5_3 as float(10,0)) as total_pop, cast(ACS_${year}_5_4 as float(10,0)) as moe FROM $EDIT_ONE" $EDIT_TWO.shp $EDIT_ONE.shp -progress
    fi
  done
